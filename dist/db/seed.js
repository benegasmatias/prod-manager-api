...
async function seed() {
    try {
        ...
        const templateRepo = data_source_1.AppDataSource.getRepository(business_template_entity_1.BusinessTemplate);
        const baseRoles = [
            {
                role: 'SUPER_ADMIN',
        ...
        const baseMachines = [
            { name: 'Ender 3 S1 #1', model: 'Creality Ender 3 S1', nozzle: '0.4mm', status: enums_2.MachineStatus.IDLE, businessId: biz3D.id },
            { name: 'Prusa MK3S+ #1', model: 'Prusa i3 MK3S+', nozzle: '0.6mm', status: enums_2.MachineStatus.PRINTING, businessId: biz3D.id },
        ];
